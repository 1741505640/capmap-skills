...
export class CapMapService extends Service {
    ...
    constructor(ctx) {
        super(ctx, 'capmap');
        ctx.connection.rpc.handle(CAPMAP_RPC_CHANNEL, (endpoint, payload, signal) => this.handleRpc(endpoint, payload, signal), { authority: 'loopback' });
    }
    ...
    async handleRpc(endpoint, payload, _signal) {
        if (endpoint === 'parse') {
            ...
            if (typeof root !== 'string' || root === '') {
                return {
                    ok: false,
                    error: { code: 'internal', message: 'capmap.parse: root 必填', details: {} },
                };
            }
            try {
                ...
                return {
                    ok: false,
                    error: { code: 'internal', message: String(err), details: {} },
                };
            }
        }
        return {
            ok: false,
            error: { code: 'internal', message: `capmap: 未知端点 ${endpoint}`, details: {} },
        };
    }
    /** 定位 docs_root：优先 `.agents/skills/capmap-system/capmap.yaml`，回退 `<root>/docs`。 */
    async resolveDocsRoot(projectRoot) {
