...
export function parseWikilinks(text) {
    ...
    while ((m = re.exec(clean)) !== null) {
        let inner = m[1].trim();
        if (inner.includes('|'))
            inner = inner.slice(0, inner.indexOf('|')).trim();
...
export function classifyNode(path) {
    const p = path.replace(/\\/g, '/');
    const name = p.split('/').pop() ?? '';
    if (p.startsWith('_archive/'))
        return 'archive';
    if (/^方案\/[^/]+\/切片\//.test(p))
        return null; // 活跃切片/规格：执行票，不入主图
    if (name.startsWith('能力底图-'))
        return 'map';
    if (p.startsWith('测试/'))
        return 'test';
    if (p.startsWith('规范/'))
        return 'norm';
    if (name.endsWith('索引.md') || INDEX_NAMES.has(name))
        return 'index';
    if (p.startsWith('方案/'))
        return 'scheme';
    return 'index';
...
/** 主入口：一组文件 → 图 JSON。 */
export function parseGraph(files) {
    const nodes = [];
    ...
    for (const f of files) {
        ...
        if (type === 'map')
            capabilities.push(...parseCapabilities(body));
    }
    const edges = [];
    ...
    for (const f of files) {
        ...
        const stem = rel.split('/').pop().replace(/\.md$/, '');
        if (!byStem.has(stem))
            continue; // 切片/规格等非节点文件不产出 wikilink 源边
        for (const target of parseWikilinks(f.content))
            addEdge(stem, target, 'wikilink');
    }
    // 语义边（默认隐藏）：能力底图 → 同主题方案/测试（目录契约推导）。
    for (const node of nodes) {
        if (node.type !== 'map' || !node.theme)
            continue;
        for (const other of nodes) {
            if (other.theme !== node.theme)
                continue;
            if (other.type !== 'scheme' && other.type !== 'test')
                continue;
            if (other.path.includes('/切片/'))
                continue;
