...
import { CAPMAP_SKILL_INSTALL_CMD } from '../setupHint.ts';
...
export function skillsAlreadyPresent(projectRoot: string): boolean {
...
function escapeWinArg(s: string): string {
...
function runCapture(
  ...
  return new Promise((resolve) => {
    ...
    const child = isWin
      ? spawn(
          `${escapeWinArg(command)} ${args.map(escapeWinArg).join(' ')}`,
          {
            cwd,
            shell: true,
    ...
    let outBuf = '';
    let errBuf = '';
    const flushLines = (kind: 'out' | 'err', chunk: string) => {
      const buf = kind === 'out' ? (outBuf += chunk) : (errBuf += chunk);
      const parts = buf.split(/\r?\n/);
      if (kind === 'out') outBuf = parts.pop() ?? '';
      else errBuf = parts.pop() ?? '';
      for (const line of parts) {
        if (line.trim()) onChunk?.(kind, line);
      }
    };
    ...
    child.on('close', (code) => {
      clearTimeout(timer);
      if (outBuf.trim()) onChunk?.('out', outBuf);
      if (errBuf.trim()) onChunk?.('err', errBuf);
      resolve({ code, stdout, stderr });
    });
  });
}

async function installByGitSparse(
  projectRoot: string,
  timeoutMs: number,
  ...
  try {
    ...
    const clone = await runCapture(
      'git',
      ['clone', '--depth', '1', '--filter=blob:none', '--sparse', GITHUB_REPO, tmp],
      cwd,
      timeoutMs,
      (_s, line) => onProgress?.(null, `[git] ${line}`),
    );
    ...
    for (const name of readdirSync(skillsSrc)) {
      if (!name.startsWith('capmap-')) continue;
...
async function installByNpxSkills(
  projectRoot: string,
  timeoutMs: number,
  onProgress?: ProgressFn,
): Promise<InstallSkillsResult> {
  ...
  const r = await runCapture('npx', args, cwd, timeoutMs, (_s, line) =>
    onProgress?.(null, `[npx] ${line}`),
  );
 ...
 */
export async function installCapmapSkills(
  projectRoot: string,
  opts?: { force?: boolean; timeoutMs?: number; onProgress?: ProgressFn },
): Promise<InstallSkillsResult> {
  const cwd = projectRoot.replace(/[/\\]+$/, '');
