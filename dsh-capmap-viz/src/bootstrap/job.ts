...
import { bootstrapCapmap } from './bootstrap.ts';
...
type Job = BootstrapJobSnapshot & { abort?: boolean };
...
function setPhase(job: Job, phase: string) {
  job.phase = phase;
  job.updatedAt = Date.now();
  pushLog(job, `→ ${phase}`);
}
...
export function startBootstrapJob(
  ...
  void (async () => {
    try {
      ...
      pushLog(
        job,
        `骨架: 新建 ${result.scaffold.created.length}，跳过 ${result.scaffold.skipped.length}`,
      );
...
export function getBootstrapJob(jobId: string): BootstrapJobSnapshot | null {
