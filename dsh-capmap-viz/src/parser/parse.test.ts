...
test('frontmatter tags（块列表）与节点分型', () => {
  ...
  const scheme = g.nodes.find(
    (n) =>
      n.id.includes('CapMap可视化-DSH插件') &&
      (n.type === 'scheme' || n.type === 'archive'),
  );
  assert.ok(scheme, '应有 CapMap 方案/归档节点');
  assert.ok(scheme!.status, '方案应有状态');
});
...
test('活跃切片跳过主图但进 sidecar；§1 能力清单', () => {
  const g = parseGraph(repoRoot);
  assert.equal(
    g.nodes.some((n) => n.path.includes('CapMap可视化-DSH插件-03')),
    false,
    '活跃切片不应入主图',
  );
  assert.ok(g.slices.some((s) => s.id.includes('CapMap可视化-DSH插件-03')), '切片应进 sidecar');
  assert.ok(g.capabilities.length >= 10, `能力数应接近 §1，实际 ${g.capabilities.length}`);
  // 无进行中方案与 §1 冲突时打架可为 0；契约是 queries 字段存在
  assert.ok(Array.isArray(g.queries.section1Fight), '应有 section1Fight 数组');
});

test('归档规格进 sidecar，不入主图、不进主题树方案组', () => {
  const g = parseGraph(repoRoot);
  const specPath = '_archive/方案/文档体系/切片/CapMap可视化-DSH插件/CapMap可视化-DSH插件-00-规格.md';
  assert.equal(
