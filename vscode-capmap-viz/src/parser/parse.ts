...
import { formatDocsRootMissingMessage } from '../setupHint.ts';
...
export class DocsRootMissingError extends Error {
  readonly code = 'docs_root_missing' as const;
...
function listMdFiles(dir: string, out: string[] = []): string[] {
  if (!existsSync(dir)) return out;
  for (const name of readdirSync(dir)) {
    if (name === '.obsidian' || name === 'node_modules') continue;
    const abs = join(dir, name);
    const st = statSync(abs);
    if (st.isDirectory()) listMdFiles(abs, out);
    else if (name.endsWith('.md')) out.push(abs);
  }
  return out;
}

function parseFrontmatter(raw: string): { tags: string[]; body: string; titleHint: string | null } {
  if (!raw.startsWith('---')) {
  ...
  let tags: string[] = [];
  try {
    const fm = yaml.load(fmText) as { tags?: unknown } | null;
    ...
    for (const line of fmText.split(/\r?\n/)) {
      ...
      if (m && fmText.includes('tags:')) tags.push(m[1].trim());
    }
  }
...
function statusFromTags(tags: string[]): string | null {
  for (const t of tags) {
    if (t.startsWith('状态/')) return t.slice('状态/'.length);
  }
  for (const t of tags) {
...
function isSliceDirPath(relPosix: string): boolean {
  return relPosix.includes('/切片/');
}

/** 切片目录下的执行件：切片票 + 规格（均不入主图，进 sidecar）。 */
function isSliceSidecarDoc(relPosix: string, tags: string[]): boolean {
  if (!isSliceDirPath(relPosix)) return false;
  return (
    tags.includes('切片') ||
    tags.includes('规格') ||
...
function themeOf(relPosix: string): string | null {
  ...
  if (iScheme >= 0 && parts[iScheme + 1] && parts[iScheme + 1] !== '切片') {
    ...
    if (seg.endsWith('.md')) return null;
    return seg;
  }
...
function schemeIdFromSlicePath(relPosix: string): string | null {
  const parts = relPosix.split('/');
...
function classify(relPosix: string, tags: string[]): CapMapNodeType {
  ...
  if (tags.includes('能力底图') || /\/能力底图-/.test(`/${relPosix}`)) return 'map';
  if (tags.includes('项目全貌')) return 'overview';
  ...
  if (tags.includes('方案') || (relPosix.startsWith('方案/') && !relPosix.includes('/切片/'))) return 'scheme';
  if (tags.includes('测试') || relPosix.startsWith('测试/')) return 'test';
  if (tags.includes('规范') || relPosix.startsWith('规范/')) return 'norm';
  if (tags.includes('切片')) return 'scheme';
  return 'index';
}

function firstParagraphSummary(body: string): string | null {
...
function resolveDocsRoot(projectRoot: string): string {
  ...
  for (const c of candidates) {
    ...
    try {
      const doc = yaml.load(readFileSync(c, 'utf8')) as { docs_root?: string } | null;
...
function parseCapabilities(mapId: string, theme: string | null, body: string): CapMapCapability[] {
  const lines = body.split(/\r?\n/);
  let inSection = false;
  const out: CapMapCapability[] = [];
  for (const line of lines) {
    ...
    if (/^-{2,}$/.test(parts[0]) || parts[0] === '能力' || parts[0].includes('---')) continue;
...
function collectLinks(body: string): { wikilinks: string[]; mdLinks: string[] } {
...
function parseBlockedBy(body: string): string[] {
  ...
  if (!m) return [];
  const rest = m[1].trim();
  if (!rest || rest === '无' || rest.startsWith('无')) return [];
  const ids: string[] = [];
...
function toSlice(doc: FmDoc): CapMapSlice {
  return {
    id: doc.stem,
    title: doc.title,
...
export function parseGraph(projectRoot: string): CapMapGraph {
  ...
  const slices: CapMapSlice[] = allDocs
  ...
  const nodes: CapMapNode[] = kept.map((d) => {
  ...
  const byRel = new Map<string, CapMapNode>();
  ...
  const addEdge = (source: string, target: string, kind: CapMapEdge['kind']) => {
    if (!byStem.has(source) || !byStem.has(target)) return;
    if (source === target) return;
    const k = `${kind}:${source}->${target}`;
    const k2 = `${kind}:${target}->${source}`;
    if (kind === 'wikilink') {
      if (edgeKey.has(k) || edgeKey.has(k2)) return;
      edgeKey.add(k);
      edgeKey.add(k2);
    } else {
  ...
  for (const d of kept) {
    ...
    const seenMdStems = new Set<string>();
    ...
    for (const w of wikilinks) {
      ...
      if (seenMdStems.has(stem)) continue;
      if (byStem.has(stem)) addEdge(d.stem, stem, 'wikilink');
    }
  }
  ...
  const capabilities: CapMapCapability[] = [];
  for (const d of kept) {
    const node = byStem.get(d.stem);
    if (!node || node.type !== 'map') continue;
