...
export function stripStatusPrefix(raw: string | null | undefined): string | null {
  if (raw == null || raw === '') return null;
  const t = raw.trim();
  return t.startsWith('状态/') ? t.slice('状态/'.length) : t;
}

function themeMatch(theme: string | null | undefined, filter: string | null): boolean {
  if (!filter || filter === '全部') return true;
  return theme === filter;
}

/** 将能力名对齐到唯一方案；失败返回 null（无法对齐）。 */
export function alignCapabilityToScheme(
  cap: CapMapCapability,
  schemes: readonly CapMapNode[],
): CapMapNode | null {
...
export function deriveFrontier(
  slices: readonly CapMapSlice[],
  themeFilter: string | null = null,
): CapMapQueryRow[] {
  const byId = new Map(slices.map((s) => [s.id, s]));
  const out: CapMapQueryRow[] = [];
...
export function deriveValidationGate(
  ...
  for (const scheme of schemes) {
    ...
    const linked = tests.filter((t) => {
      ...
      const mentions =
        (t.summary && t.summary.includes(scheme.id)) ||
...
export function deriveSection1Fight(
  capabilities: readonly CapMapCapability[],
  nodes: readonly CapMapNode[],
  themeFilter: string | null = null,
): CapMapQueryRow[] {
  ...
  for (const cap of capabilities) {
    ...
    if (!scheme) continue; // 无法对齐：不进打架、不单列
