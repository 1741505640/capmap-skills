...
| 12 | 闭环含 **开发标识 / 测试 / 使用规范**：方案≠已落地；已开发=方案范围落地+底图§1/§2（不等人测）；标完已开发须询问是否转测试；长期契约进 `规范/`；测试进 `测试/` |
...
| 26 | **Inbox** = 文档首页一节；行必须有可执行 **开干指令**；点名当场执行后删行。碎片无 `状态/*`、不进 §1。不是台账 | 对话摘录表；独立 Inbox.md；每会话自动倒表 |
| 27 | **capmap-debug**：运行时修锅旁路；强制 5 行清单；无新证据最多 3 次改动后停损并建议 Inbox；不进状态机、不改切片 Tag | 把 debug 写进方案主状态；无停损盲续；与 gate 互斥 |
| 28 | **小需求收尾降税**：`已开发` = Agent 对照方案判定范围已落地 + §1/§2；**禁止**「请先自测再说已开发」「测完可以说一声再走 capmap-dev」。标完后**必须询问**是否转测试（勿静默结束）；用户同意再 test 拍 1。用户本回合已点名转测则可直接链式。`已验证` 仍须人测或免测 | 编码完停成等人回「测过了」才标已开发；标完已开发静默结束不问下一步；未经询问自动写测试文；Agent 自评已验证 |
| 29 | **capmap-handoff**：进行中换窗冷启动；落盘 `<docs_root>/_handoff/`；交棒点头才写；接棒后移 `_consumed/`（最近 5）；同 scope 活跃 1 份；**不**改 `状态/*`；**禁止**把进行中工作写成 Inbox | 用 Inbox 塞半成品；交接进方案正文；静默落盘；接棒后留活跃双窗歧义；引入交接专用状态 Tag |
...
- 编码收尾「请先自测、测过再说已开发」「测完可以说一声再走 capmap-dev」（人测不挡已开发）
- 标完 `已开发` 静默结束、不问是否转测试
- 未经询问（且用户未点名）自动写测试文
- 进行中换窗写成 Inbox 行；交接文挂 `状态/*` 或当第二套方案
