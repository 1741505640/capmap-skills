...
| 28 | **小需求收尾降税**：`已开发` = Agent 对照方案判定范围已落地 + §1/§2；**禁止**「请先自测再说已开发」。标完后**必须询问**是否转测试（勿静默结束）；用户同意再 test 拍 1。用户本回合已点名转测则可直接链式。`已验证` 仍须人测或免测 | 编码完等人回「测过了」才标已开发；标完已开发静默结束不问下一步；未经询问自动写测试文；Agent 自评已验证 |
